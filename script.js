...
async function checkAdminBlock() {
    try {
        const response = await fetch(GOOGLE_SCRIPT_URL, {
            method: 'POST',
            body: JSON.stringify({
                action: "check_block", // Yashirin tekshiruv buyrug'i
                deviceId: getOrCreateDeviceId()
            })
        });
        const result = await response.json();
        if (result.blocked) {
            // Agar bazada block qilingan bo'lsa, xotirani o'chirib, haydab yuboramiz
            ...
            location.reload(); // Sahifani yangilab Auth oynaga otish
        }
    } catch (e) {
